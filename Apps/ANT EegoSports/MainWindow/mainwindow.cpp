...
// start/stop the eego sports connection
void MainWindow::link() {
	if (reader != NULL) {
		...
		try {
			...
			connect(reader, SIGNAL(errorMsg(QString)), this, SLOT(threadError(QString)));
			thread->start();
		}
		catch (std::exception &e) {
			// try to decode the error message
			std::string msg = "Could not query driver message because the device is not open";
			QMessageBox::critical(this, "Error", ("Could not initialize the eegosport interface: " + (e.what() + (" (driver message: " + msg + ")"))).c_str(), QMessageBox::Ok);
			return;
		}
...
void MainWindow::threadTimeout() {
	QMessageBox::critical(this, "Error", (std::string("Error: eego sport amp connection timed out")).c_str(), QMessageBox::Ok);
}

void MainWindow::threadError(QString message) {
...
void Reader::read() {
	...
	try {
		...
		ampFound = false;
		emit errorMsg("Amp not found or license file not present. Please connect the amplifier and make sure that the amp is turned on and a license file is present in 'My Documents/eego/'");
		//emit timeout();
	}
	catch (exceptions::notConnected) {
		emit errorMsg("Amp connection lost");
	}
	catch (std::exception &e) {
	}
	if (ampFound) {
